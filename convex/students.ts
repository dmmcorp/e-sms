...
export const getStudentById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const enrollmentWithSection = await asyncMap(
      enrollments,
      async (enrollment) => {
        const section = await ctx.db.get(enrollment.sectionId);
        if (section === null) return undefined;

        const selectedSubjects = await asyncMap(
          enrollment.subjects,
          async (id) => {
    ...
    const currentSection = enrollmentWithSection
...
export const sectionStudents = query({
  ...
  handler: async (ctx, args) => {
    ...
    const initStudents = await ctx.db
      .query("sectionStudents")
      .filter((q) => q.eq(q.field("sectionId"), args.sectionId))
      .collect();
    ...
    const students = await asyncMap(initStudents, async (data) => {
      ...
      const classRecord = await ctx.db
        ...
        .withIndex("by_teachingLoadId", (q) =>
          q.eq("teachingLoadId", args.teachingLoadId)
        )
      ...
      const written = await ctx.db
        ...
        .withIndex("by_classRecordId", (q) =>
          q.eq("classRecordId", classRecord._id)
        )
        .collect();
      ...
      const sortedWritten = written.sort(
        (a, b) => a.assessmentNo - b.assessmentNo
      );
      ...
      const isSubmitted =
        classRecord.needsIntervention !== undefined &&
        classRecord.needsIntervention !== null;
...
export const needsIntervention = query({
  ...
  handler: async (ctx, args) => {
    ...
    const filteredStudents = students
      .filter((s) => s?.classRecord !== null)
      .filter((s) => s?.classRecord?.needsIntervention === true);
...
export const getSubjects = query({
  ...
  handler: async (ctx, args) => {
    const subjectsWithGrades = await getStudentSubjects(
      args.sectionSubjects,
      ctx,
      args.studentId
    );
    const filterSubjectsWithGrades = subjectsWithGrades?.filter(
      (s) => s !== null
    );
...
const getStudentSubjects = async (
  ...
  const classRecordsWithTeachingLoad = await asyncMap(
    classRecords,
    async (record) => {
  ...
  const subjectWithGrades = await asyncMap(
    sectionSubjects,
    async (subjectId) => {
      ...
      if (subject.subjectName.toLowerCase() === "mapeh") {
        ...
        const componentLoads = teachingLoads.reduce(
          (acc, load) => {
        ...
        const components = await Promise.all(
          Object.entries(componentLoads).map(async ([component, loads]) => {
            ...
            for (const load of loads) {
              ...
              const record = filteredCR.find(
                (r) => r.teachingLoad._id === load._id
              );
        ...
        for (const record of filteredCR) {
          if (record.teachingLoad.subjectTaughtId === subjectId) {
            const quarter = record.teachingLoad.quarter?.replace(
              " quarter",
              ""
            ) as Quarter;
...
const getSectionSubjects = async (
  ...
  const classsRecords = await ctx.db
    .query("classRecords")
    .withIndex("by_studentId", (q) => q.eq("studentId", studentId))
    .collect();
...
export const getStudentSubjectsByEnrollment = query({
  ...
  handler: async (ctx, args) => {
    ...
    const studentEnrollments = await asyncMap(enrollments, async (e) => {
      ...
      if (
        !args.isSHS &&
        (section.gradeLevel === "Grade 11" || section.gradeLevel === "Grade 12")
      )
        return null;
    ...
    filteredStudentGrades.forEach((e) => {
      if (e && (e.gradeLevel === "Grade 11" || e.gradeLevel === "Grade 12")) {
        gradeLevelMap[`${e.gradeLevel} - ${e.semester}`] = e;
      }
      if (e && e.gradeLevel !== "Grade 11" && e.gradeLevel !== "Grade 12") {
    ...
    const organizedGrades: OrganizedGrade[] = gradeLevels.map((level) => {
      if (gradeLevelMap[level]) {
        ...
        if (
          (level.includes("Grade 11") || level.includes("Grade 12")) &&
          data.semester
        ) {
          const semester = level.includes("1st semester")
            ? "1st semester"
            : "2nd semester";
