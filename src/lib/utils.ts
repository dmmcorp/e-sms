...
import {
  ...
  transmutationTableSHS2,
  transmutationTableSHSCore2,
} from "./constants";
...
export function convertToTransmutedGrade(
  ...
  const foundEntry = transmutationTable.find(
    (entry) => initialGrade >= entry.min && initialGrade <= entry.max
  );
...
export function convertSemesterToNumber(semester: string | undefined): string {
