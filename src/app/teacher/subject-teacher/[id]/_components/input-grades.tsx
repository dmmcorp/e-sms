...
function ClassRecordTemplate({
  ...
  const [selectedTransmutedGrade, setSelectedTransmutedGrade] = useState<
    number | undefined
  >(60);
  const [studentScores, setStudentScores] = useState<
    StudentScoresType | undefined
  >();
  ...
  const isMapeh =
    teachingLoad.subjectTaught.subjectName.toLowerCase() === "mapeh";
  ...
  return (
    <div className="min-w-[1000px] text-primary">
      ...
      <div className="flex max-w-full hover:bg-gray-200 ">
        ...
        <button
          onClick={() => handleDialogOpen("Written Works", "highest scores")}
          className="w-[27%] uppercase border border-x-0 border-black  text-sm flex justify-center items-center font-semibold text-center"
        >
          {Array.from({ length: 10 }).map((_, index) => (
            <h1
              key={index}
        ...
        <button
          onClick={() =>
            handleDialogOpen("Performance Tasks", "highest scores")
          }
          className="w-[28%] hover:bg-gray-200  uppercase border border-black border-x-0  text-sm flex justify-center items-center font-semibold text-center"
        >
          {Array.from({ length: 10 }).map((_, index) => (
            <h1
              key={index}
        ...
        {learningMode?.toLowerCase() === "face to face" && (
          <button
            onClick={() => handleDialogOpen("Major Exam", "highest scores")}
            className="w-[8%] uppercase border border-x-0 border-black  text-sm leading-relaxed grid grid-cols-3 justify-center items-center font-semibold text-center"
          >
      ...
      {males.map((student, index) => {
        ...
        const wwTotalScore = calculateTotalScore(
          student.written.map((w) => w.score)
        );
        setSelectedFullName(fullName);
        const wwPercentageScore = calculatePercentageScore(
          wwTotalScore,
          wwTotal
        );
        ...
        const ptTotalScore = calculateTotalScore(
          student.performance.map((p) => p.score)
        );
        const ptPercentageScore = calculatePercentageScore(
          ptTotalScore,
          ptTotal
        );
        ...
        const meTotalScore = calculateTotalScore(
          student.exam.map((e) => e.score)
        );
        const mePercentageScore = calculatePercentageScore(
          meTotalScore,
          meTotal
        );
        ...
        return (
          <div key={student._id} className="flex max-w-full hover:bg-gray-200 ">
            ...
            >
              {Array.from({ length: 10 }).map((_, wwindex) => (
                <h1
                  ...
                  className="w-[7%] h-full uppercase border border-b-0 border-black first:border-l-2   text-[0.6rem] flex justify-center items-center font-semibold text-center"
                >
                  {student.written.find((w) => w.assessmentNo === wwindex + 1)
                    ?.score === 0
                    ? "0"
                    : student.written.find(
                        (w) => w.assessmentNo === wwindex + 1
                      )?.score || ""}{" "}
            ...
            <button
              ...
              className="w-[28%] uppercase border-b-black border-b text-sm flex justify-center items-center font-semibold text-center"
            >
              {Array.from({ length: 10 }).map((_, ptindex) => (
                <h1
                  ...
                  className="w-[7%] h-full uppercase border border-b-0 border-black  text-[0.6rem] flex justify-center items-center font-semibold text-center"
                >
                  {student.performance.find(
                    (p) => p.assessmentNo === ptindex + 1
                  )?.score === 0
                    ? "0"
                    : student.performance.find(
                        (p) => p.assessmentNo === ptindex + 1
            ...
            >
              <h1 className="h-full uppercase border border-black border-b-0 border-t-0  text-xs flex justify-center items-center  font-semibold text-center">
                {initialGrade === 0 ? "" : initialGrade.toFixed(1)}
              </h1>
              {student.classRecord.needsIntervention ? (
                <CustomTooltip
                  trigger={
                    <h1
                      className={cn(
                        (interventionGrade ?? transmutedGrade) <= 74 &&
                          "text-red-500",
                        "h-full uppercase border border-black border-b-0 border-t-0  text-xs flex justify-center items-center  font-semibold text-center"
                      )}
                    >
                      {transmutedGrade === 60
                        ? ""
                        : (interventionGrade?.toFixed(0) ??
                          transmutedGrade.toFixed(0))}
                    </h1>
                  }
                  initialGrade={transmutedGrade.toString()}
                  interventionRemarks={
                    interventionRemarks ?? "No assigned Remarks"
                  }
