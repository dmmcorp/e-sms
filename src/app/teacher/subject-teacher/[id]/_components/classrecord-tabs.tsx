...
function ClassrecordTabs({ teachingLoad }: ClassrecordTabsProps) {
  ...
  const createMapehClassRecords = useMutation(
    api.teachingLoad.createMapehClassRecords
  );
  ...
  const handleComponentChange = (value: string) => {
    ...
    const selectedLoad = mapehLoads?.find(
      (load) => load.subComponent === value
    );
    if (selectedLoad) {
  ...
  useEffect(() => {
    if (
      isMapeh &&
      teachingLoad &&
      teachingLoad.classRecords.length === 0 &&
      sectionStudents &&
      enrollments
    ) {
  ...
  return (
    <div>
      <Tabs
        defaultValue={"class-record"}
        className="w-full relative space-y-5 shadow-none"
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-col gap-2">
            <TabsList className="w-fit md:block">
              <TabsTrigger
                value="class-record"
                className="font-medium text-[0.5rem] md:text-sm shadow-md border-b-2 data-[state=active]:border-b-primary data-[state=active]:text-primary"
              >
                Class Record
              </TabsTrigger>
              <TabsTrigger
        ...
        <TabsContent
          value="class-record"
          className="border-2 p-5 overflow-auto"
        >
