            ...
            <div className="w-full">
              ...
              <div className="grid grid-cols-2 py-2">
                ...
                <div className="text-right">
                  ...
                  {load.subject?.subjectName.toLowerCase() === "mapeh" &&
                    load.subComponent && (
                      <div className="flex items-center justify-end gap-2 mt-1">
                        <div className="h-2 w-2 rounded-full bg-primary"></div>
                        <span className="text-sm text-muted-foreground">
                          {load.subComponent} Component
                        </span>
                      </div>
                    )}
                </div>
              ...
              <div className="grid grid-cols-2 text-muted-foreground gap-3 mb-5">
                ...
                <div className="">
                  ...
                  <CustomTooltip
                    trigger={
                      <h1 className="">Dropped: {load.droppedStud.length}</h1>
                    }
                    content={
                      <div className="flex flex-col items-start justify-start min-h-56 min-w-56 p-2">
                        <h1 className="text-sm font-semibold text-center w-full mb-2">
                          Dropped
                        </h1>
                        {load.droppedStud.length !== 0 ? (
                          load.droppedStud.map((student, index) => (
                            <div
                              key={"dropped" + student.student?._id}
                ...
                <div className="">
                  ...
                  <CustomTooltip
                    trigger={
                      <h1 className="">
                        Returning: {load.returningStud.length}
                      </h1>
                    }
                    content={
                      <div className="flex flex-col items-start justify-start min-h-56 min-w-56 p-2">
                        ...
                        {load.returningStud.length !== 0 ? (
                          load.returningStud.map((student, index) => (
                            <div
                              key={"returning" + student.student?._id}
                              className="flex justify-between w-full"
                            >
                              <h3 className="capitalize line-clamp-1 text-ellipsis">
                                {index + 1}.{student.student?.lastName},{" "}
                      ...
                      <div className="flex flex-col items-start justify-start min-h-56 min-w-56 p-2">
                        ...
                        {load.needsInterventions.length !== 0 ? (
                          load.needsInterventions.map((student, index) => (
                            <div
                              key={"intervention" + student.student?._id}
                              className="flex justify-between w-full"
                            >
                              <h3 className="capitalize line-clamp-1 text-ellipsis">
                                {index + 1}.{student.student?.lastName},{" "}
              ...
              <Table className="w-full border-x-gray-50 border-x shadow-md h-[100px] max-h-[100px] overflow-auto">
                <TableHeader className="bg-primary ">
                  <TableRow className="font-semibold">
                    <TableHead className="text-muted max-w-[25%]">
                      Student Name
                    </TableHead>
                    <TableHead className="text-muted max-w-[25%]">
                      Interventions Done
                  ...
                  {load.needsInterventionsAll.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center ">
                        No failed students for this quarter.
                      </TableCell>
                    </TableRow>
                  ) : (
                    load.needsInterventionsAll.map((load) => (
                      <TableRow key={load.studentId}>
                        ...
                        <TableCell>
                          {load.interventionUsed &&
                            load.interventionUsed.map((intervention) => (
                              <div key={intervention}>{intervention}</div>
                            ))}
                        </TableCell>
                        ...
                        <TableCell className=" max-w-[25%] text-wrap">
                          {load.teachingLoad.quarter}{" "}
                          {load.teachingLoad.semester
                            ? `- ${load.teachingLoad.semester}`
                            : ""}
                        </TableCell>
                      </TableRow>
