    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
            ...
            <div className="space-x-2 space-y-2">
                <Label className=''>Selected subjects: <span>{subjects.length}</span></Label>
                    {currentSection?.sectionSubjects.map((subject, index) => (
                        <Badge  
                            key={`${subject.subject?._id}-${index}`}
                            className={cn(subject.subject?._id && subjects.includes(subject.subject._id) ? "" : "text-black bg-muted shadow-md ", "hover:cursor-pointer transition-colors duration-300 ease-in-out p-0")}
                        >
                        <label className='hover:cursor-pointer size-full px-2 py-1 rounded-md'>
                        <Checkbox
                            checked={subject.subject?._id ? subjects.includes(subject.subject._id) : false}
                            onCheckedChange={(checked) => {
                                if(checked && subject.subject?._id){
                                    setSubjects([...subjects, subject.subject._id])
                                } else {
                                    const remove = subjects.filter(s => s !== subject.subject?._id)
                                    setSubjects(remove)
                                }
                            }}
                            className='hidden'
