...
import { MapehComponent, MapehMainSubject, Quarter, QuarterAverages, QuarterGrades, SectionStudentsType, SubjectType, ValidCounts } from '@/lib/types'
...
function PromoteDialog({
    ...
    const isSHS = student.section.gradeLevel === "Grade 11" || student.section.gradeLevel === "Grade 12";
    ...
    function getSubjectsBelowPassing(): { subject: SubjectType; average: number | null }[] {
        if (!subjects || subjects.length === 0) return [];

        return organizedSubjects
            .filter(subject => {
                // Exclude MAPEH components and subjects with no grades
                if ('isMapehComponent' in subject && subject.isMapehComponent) return false;
                if (!subject.grades || Object.values(subject.grades).every(grade => grade === undefined)) return false;
                return true;
            })
            .map(subject => {
                const average = calculateQuarterlyAverage('isMapehMain' in subject && subject.isMapehMain ?
                    subject.grades :
                    {
        ...
        try {
            ...
            }), 
            {
                ...
                success: (data) => {
                    ...
                    return <div>Success: <span className='capitalize'> {data?.promotionType}</span></div>
                },
                error: (error) => {
                    const errorMes = error.data ? error.data : "Something went wrong!"
                    return <div>Invalid: <span className='capitalize'> {errorMes}</span></div> 
                }
            }
            )
            ...
            isSHS ? (
                <>
                    <div className="space-y-3">
                        <p><strong className='capitalize'>{fullName}</strong> has <strong>failed ({failedSubjects.length})</strong> of his subjects.</p>
                        <h1>Failed Subject(s):</h1>
                        {failedSubjects.map((fs) => (
                            <div key={fs.subject._id} className="">
                                <h1 className='pl-5'>- <strong>{fs.subject.subjectName} - ({fs.average})</strong></h1>
                            </div>
                        ))}
