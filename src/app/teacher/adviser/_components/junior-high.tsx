         ...
         trigger={
          <Card key={load._id} className="">
            <CardContent>
              <div className="">
                <div className="">
                  <Chart classRecords={load.classRecords} label={load.subject.subjectName}/>
                </div>
              </div>
            </CardContent>
          </Card>
         ...
         content={
           <div className="flex items-center justify-center min-h-56 min-w-56">
             {load.needsInterventions.length !== 0 ? load.needsInterventions.map((student,index)=>(
               <h3 key={"intervention" + student.student?._id}>{index + 1}.{student.student?.lastName}, {student.student?.firstName} {student.student?.middleName.charAt(0)} </h3>
             )): (
